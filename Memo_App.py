...
def Ui():
    ...
    while True:
        ...
        if choice == '1':
            title = input("제목 입력: ")
            content = input("내용 입력: ")
            success = controller.create_memo(title, content)
            if success:
            ...
            old_content = controller.get_memo(title)
            if old_content is None:
                print("해당 제목의 메모가 존재하지 않습니다.")
            else:
                print(f"\n현재 내용:\n{old_content}")
                new_content = input("\n새 내용 입력: ")
                success = controller.update_memo(title, new_content)
                if success:
            ...
            titles = controller.get_all_memo_titles()
            if not titles:
                print("저장된 메모가 없습니다.")
            else:
                print("저장된 메모 목록:")
            ...
            content = controller.get_memo(title)
            if content is None:
                print("해당 제목의 메모가 존재하지 않습니다.")
            else:
                print(f"\n[{title}]")
